...
export async function saveFirebaseLevelProgress(
  ...
  try {
    ...
    // Update highestLevel and leaderboard on user profile
    ...
    if (userDoc.exists()) {
      ...
      if (completed && levelId + 1 > currentHighestLevel) {
        await updateDoc(userRef, {
          highestLevel: levelId + 1,
...
/**
 * Save user game stats to Firestore
 */
export async function saveFirebaseUserStats(userId, stats) {
  try {
...
/**
 * Get user game stats from Firestore
 */
export async function getFirebaseUserStats(userId) {
  try {
    const statsRef = doc(db, 'user_stats', userId);
    const statsDoc = await getDoc(statsRef);
 ...
 */
export async function saveFirebaseAchievements(userId, achievements) {
  try {
    const achRef = doc(db, 'user_achievements', userId);
    await setDoc(achRef, {
      userId,
      achievements,
      updatedAt: Timestamp.now(),
 ...
 */
export async function getFirebaseAchievements(userId) {
  try {
    const achRef = doc(db, 'user_achievements', userId);
    const achDoc = await getDoc(achRef);
    
    if (achDoc.exists()) {
      return achDoc.data().achievements || [];
    }
    return [];
  } catch (error) {
