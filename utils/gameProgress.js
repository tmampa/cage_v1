...
export async function saveLevelProgress(userId, levelId, score, completed) {
  try {
    ...
    if (progressError && progressError.code === '42P01') {
      console.error("The level_progress table doesn't exist. Please run the setup SQL script in the Supabase dashboard.");
      return { error: "Database tables not set up. Please contact an administrator." };
    }
  ...
  try {
    ...
    try {
      ...
      if (checkError && checkError.code === '42P01') {
        console.error("The unlocked_levels table doesn't exist. Please run the setup SQL script in the Supabase dashboard.");
